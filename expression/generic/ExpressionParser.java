...
public class ExpressionParser<T extends Evaluative<T>> extends BaseParser implements Parser<T> {
    ...
    T type;
    public ExpressionParser(T type) {
        this.type = type;
    }
    ...
    private CommonExpression<T> getExpression(int level) {
        if (level == lowestLevel) {
            return getPrimitiveExpression();
        }
        CommonExpression<T> expression = getExpression(level - 1);
    ...
    private Const<T> getConst(boolean isNegative) {
        ...
        try {
            ...
            return new Const<>(type.parse(val.toString()));
        } catch (NumberFormatException e) {
            throw error("Illegal constant: '" + val.toString() + "'");
        }
    }

    private Variable<T> getVariable() {
    ...
    private CommonExpression<T> getPrimitiveExpression() {
        if (test('-')) {
            if (between('0', '9')) {
                return getConst(true);
            } else {
                return new Negate<>(getExpression(lowestLevel));
    ...
    private CommonExpression<T> makeExpression(Operator operator, CommonExpression<T> first, CommonExpression<T> second) {
        switch (operator) {
            case ADD:
